...
| **cloud-dev proxy** | `apps/base/legacy-proxy/` | `cloud-dev.savar.de` bleibt auf `nc01-dev` (`192.168.2.220`), läuft aber über Traefik TLS -> Cluster-LB `.246` -> nginx-inc. Hauptdienst wird zu HTTPS/443 re-encryptet; `/push/` geht per WebSocket und Strip-Prefix an notify_push/7867. EndpointSlices nach dem ArgoCD-Sync einmalig manuell anwenden; `nc02-dev` (`.221`) erst nach Erreichbarkeits- und Release-Prüfung wieder aufnehmen. |
