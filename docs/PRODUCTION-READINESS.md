...
| **mailman** | `apps/base/mailman/` | Upgrade auf 0.5.2 ist produktiv; PostgreSQL-Zeitspalten wurden repariert, Web-Startup-Limit nach zwei OOMs auf 2 GiB erhöht. Core, LMTP und der ausgehende Rundlauf sind verifiziert. Web bleibt für den Volltext-Neuaufbau auf null; ein getestetes lokales Haystack-Template begrenzt dabei nur den indexierten Body auf 1 MiB. Nach validiertem Index Web wieder starten und öffentliche Seiten, Suche, Queue sowie Restarts erneut prüfen. |
