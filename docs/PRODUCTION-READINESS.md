...
| **nextcloud-yealink-phonebook** | `apps/base/nextcloud-yealink-phonebook/` | Liest das Nextcloud-Adressbuch alle 15 Minuten per CardDAV und stellt Yealink-XML unter einem zufälligen 256-Bit-Pfadtoken bereit. Eine IP-Allowlist ist wegen Source-NAT am Ingress-LB nicht belastbar; der Telefonbuchpfad liefert ohne Token 404 und Access-Logging ist für diesen Ingress deaktiviert, damit der Token nicht in Logs landet. Nach dem ersten ArgoCD-Sync die Remote-Telefonbuch-URL auf dem Yealink konfigurieren und Abruf sowie Namensauflösung bei Anrufen prüfen. |
