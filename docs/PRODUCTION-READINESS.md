...
| **ciso-assistant** | `apps/base/ciso-assistant/` | GRC-Plattform, v. a. fuer das Verzeichnis von Verarbeitungstaetigkeiten (Art. 30 DSGVO). NUR intern auf `grc.jit.services`, per IP-Allowlist im Ingress erzwungen (RFC1918 + NetBird), weil 443 vom Router auf denselben LB zeigt. Postgres via CNPG (`ciso-assistant-pg`, Backup `s3://backups/cnpg-ciso-assistant/`). **Vor dem Merge:** Garage-Key `cnpg-ciso-assistant` anlegen und `backup-s3.sops.yaml` fuellen (noch REPLACE_ME); Keycloak-Client `ciso-assistant` im Realm `bgt` anlegen, SSO dann in der App-UI eintragen. Evidenzen im Ceph-RGW-Bucket `ciso-assistant-evidence` (RGW-User mit 20-GiB-Quota), Mail ueber mx02 per IP-Relay -- neue Nodes in `mx_gateway_trusted_clients` nachziehen. Offen: Offsite-Kopie des Evidenz-Buckets (rclone nach Garage); Resources nach einer Woche messen. Ablauf: `docs/runbooks/ciso-assistant-setup.md`. |
