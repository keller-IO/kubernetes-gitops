...
| **paperless-ngx** | `apps/base/paperless-ngx/` | Externe Domain `paperless.savar.de` ist im Overlay gesetzt; TLS endet während der Migration am Legacy-Traefik. Version 2.20.15 ist der verpflichtende Zwischenstand vor 3.x; nach erfolgreichem Sync das separate 3.x-Runbook abarbeiten (`docs/runbooks/paperless-v3-upgrade.md`). Admin + SECRET_KEY; OIDC-JSON `server_url`/`secret`; CephFS-RWX für media/consume bestätigen. |
