...
- [x] **Globaler NGINX-Default**: `client-max-body-size: 10m` (Fallback, App-Ingresses überschreiben
      ...
      global aktiviert: Cluster-Zertifikate werden vorab ausgestellt, die Redirects bleiben aber
      bis zum direkten Router-Cutover deaktiviert; einige externe Provider-Zonen sind weiterhin
      HTTP-only. HSTS erst nach vollständigem Cluster-direktem-TLS-Cutover nachziehen.
...
- [x] Sechs `_acme-challenge`-CNAMEs für `imcor.de` und `jonaks.com` bei United
      ...
      waren für alle sechs Delegationen erfolgreich; auch der vorübergehend
      negative Cache für `db.imcor.de` auf `1.1.1.1` ist abgelaufen.
...
| **phpmyadmin** | `apps/base/phpmyadmin/` | `dbadmin.jit.services`, `phpmyadmin.savar.de` und `phpmyadmin.jit-creatives.de` teilen ein Cluster-TLS-Secret; der X-Forwarded-Proto-basierte Redirect bleibt hinter Traefik kompatibel. Zugriff absichern (IP-Allowlist oder separater Auth-Proxy); nur dedizierte DB-User statt Root verwenden; Default-DB-Host `kimai-mariadb.kimai.svc.cluster.local` prüfen; weitere Ziele als FQDN eintragen. |
