...
| **paperless-ngx** | `apps/base/paperless-ngx/` | Externe Domain `paperless.savar.de` ist im Overlay gesetzt; TLS endet während der Migration am Legacy-Traefik. Scanner-Inbox `//192.168.2.75/scanner` ist via SMB-CSI als Consume-Verzeichnis eingebunden und wird alle 10 Sekunden abgefragt; erfolgreich importierte Dateien werden von Paperless aus der Freigabe entfernt. Version 2.20.15 ist der verpflichtende Zwischenstand vor 3.x; nach erfolgreichem Sync das separate 3.x-Runbook abarbeiten (`docs/runbooks/paperless-v3-upgrade.md`). Admin + SECRET_KEY; OIDC-JSON `server_url`/`secret`; CephFS-RWX für media bestätigen. |
...
| **mailman** | `apps/base/mailman/` | Upgrade auf 0.5.2 ist produktiv; PostgreSQL-Zeitspalten wurden repariert, Web-Startup-Limit nach zwei OOMs auf 2 GiB erhöht. Core, Web, LMTP, öffentliche Seiten und geschlossene Registrierung sind verifiziert; ausgehende Rundlauf-Zustellung bleibt separat zu prüfen. |
